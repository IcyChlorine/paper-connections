...
1. Use the default capture with no extra flags to save every visible Zotero top-level window into `tools/screenshots/`.
2. Use `--window-query <text>` to narrow within Zotero windows by title or class, for example `Plugins` or `Preferences`.
3. Use `--single-window` only when one best-match window is enough.
4. Use `--list-windows` only when target matching is unclear or the capture result needs debugging.
...
- List current Zotero windows for debugging:
...
- The default capture saves every visible Zotero top-level window and should be the first-pass workflow for faster UI iteration.
...
- `--list-windows` is a fallback for query debugging, not the default capture step.
