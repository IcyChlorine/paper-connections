...
- Graph interactions
  ...
  - Topic-menu `Export as SVG` first opens an export-settings dialog (intro text `SVG 导出设置：`; `包含背景网格` and inline `边距（像素）：` input in zh locale), then opens a save dialog and writes an SVG of the content bounds plus margin.
